...
import { getPersistentSettings } from '@/lib/persistence';
...
const SCWLogoSVG = ({ className }: { className?: string }) => (
  <svg viewBox="0 0 180 60" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
    <defs>
    ...
    <g filter="url(#neon-glow)">
      ...
      <text x="54" y="38" fontFamily="monospace" fontWeight="bold" fontSize="16" fill="white" className="animate-pulse">{"</>"}</text>
...
export function Navbar() {
  ...
  useEffect(() => {
    updateLogo();
    window.addEventListener('storage_update', updateLogo);
    return () => window.removeEventListener('storage_update', updateLogo);
  }, []);
    ...
    <nav className="sticky top-0 z-50 w-full border-b border-white/5 bg-background/80 backdrop-blur-xl">
      <div className="container mx-auto px-4 flex h-20 md:h-28 items-center justify-between">
        <Link href="/" className="flex items-center gap-4 group">
          <div className="relative h-12 w-32 md:h-20 md:w-48 transition-transform duration-300 group-hover:scale-105 flex items-center">
            {logoUrl ? (
              <img src={logoUrl} alt="SCW Logo" className="h-full w-auto object-contain max-h-[60px]" />
            ) : (
              <SCWLogoSVG className="w-full h-full" />
            )}
          </div>
        </Link>
        ...
        <div className="hidden lg:flex items-center gap-16">
          <div className="flex items-center gap-12">
            <Link
              href="/"
              className={cn(
                "text-4xl font-bold transition-all hover:text-primary hover:translate-y-[-2px]",
              ...
              <div className="flex flex-col gap-12 mt-20 h-full">
                ...
                <div className="mt-auto pb-20 border-t border-white/10 flex flex-col gap-8 pt-10">
                  ...
                  <div className="flex gap-10 justify-center">
                    <a href={`https://wa.me/${whatsappNumber}`} target="_blank" rel="noopener noreferrer" className="p-5 bg-white/5 rounded-full text-[#25D366]">
                      <WhatsAppIcon className="w-10 h-10" />
                    </a>
                  </div>
                </div>
